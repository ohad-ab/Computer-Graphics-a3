...
void  MovableGLM::MyRotate1(float angle,const glm::vec3 &vec,int mode)
{
	if (mode == 0)
		rot1 = glm::rotate(glm::mat4(1), angle, vec)*rot1;
	else
		rot1 = glm::rotate(rot1,angle,glm::vec3(glm::transpose(rot1)*glm::vec4(vec,0)));
}
